from OECycle import CoolantBleedCycle, OpenExpanderCycle
...
@dataclass
class SE21D_V2(SE21D):
    @cached_property
    def minimum_required_coolant_mass_flow(self):
        # This state is used instead of cooling_inlet_flow_state to prevent recursion (mass flow is dependent on splitters and splitters need this input)
        coolant_state = FlowState(propellant_name=self.fuel_initial_flow_state.propellant_name,
                                  temperature=32.375,
                                  pressure=self.fuel_initial_pressure + self.delta_p_fuel_pump + self.delta_p_fuel_pump2,
                                  mass_flow=0,
                                  type='fuel')
        # Copy of CCS with new state
        ccs = CoolingChannelSection(inlet_flow_state=coolant_state,
    ...
    def post_fuel_pump_splitter(self):
        return Splitter(inlet_flow_state=self.fuel_pump.outlet_flow_state,
                        required_outlet_mass_flows=(self.coolant_mass_flow,),
                        outlet_flow_names=('secondary_pump', 'chamber'))

@dataclass
class SE21D_PressureExact_V2(SE21D_V2, SE21D_PressureExact):
    pass
...
@dataclass
class SE21D_PressureExact_V3(SE21D_V3):
...
if __name__ == '__main__':
    ...
    # exact, estimated = SE21D_PressureExact, SE21D
    # exact, estimated = SE21D_PressureExact_V2, SE21D_V2
    exact, estimated = SE21D_PressureExact_V3, SE21D_V3


    for EngineClass, pressurething in zip((exact, estimated), ('exact', 'estimated')):
