...
def get_engine_plot_data(engine_cycle: type(EngineCycle), func: Callable[[type(EngineCycle)], float], thrust: float = 100e3,
                         burn_time: float = 300, p_cc_range=range(3, 11), is_frozen: bool = False, verbose: bool = False,
...
def plot_isp_ratio(savefig=False,
                   dir_name: str = '', kwak: bool = False):
    p_ccs = range(3, 11)
    gg_cycle = KwakFixGasGeneratorCycle if kwak else GasGeneratorCycle
    ep_cycle = KwakFixElectricPumpCycle if kwak else ElectricPumpCycle
    gg_isps = get_engine_plot_data(engine_cycle=gg_cycle,
                                   func=lambda x: x.overall_specific_impulse,
                                   extra_args=args.gg_arguments)
    ep_isps = get_engine_plot_data(engine_cycle=ep_cycle,
                                   func=lambda x: x.overall_specific_impulse,
                                   extra_args=args.ep_arguments)
...
def ratio_plot(func, ylabel, title, pngname, engines_dict={},
    ...
    p = []
    fig, ax = plt.subplots()
    arguments = args.base_arguments.copy()
    arguments['is_frozen'] = is_frozen
    ...
    for burn_time, colour in zip(burn_times, colors):
        for thrust, linestyle in zip(thrusts, linestyles):
            ...
            if engines_dict:
                ...
                ep_datas = get_engine_plot_data(ep_cycle, func, args=arguments, extra_args=ep_arguments)
                gg_datas = get_engine_plot_data(gg_cycle, func, args=arguments, extra_args=gg_arguments)
...
def plot_all_ratio_plots(short: bool = False, is_frozen: bool = False, **kwargs):
    arguments = args.base_arguments.copy()
    arguments['is_frozen'] = kwargs['is_frozen'] = is_frozen
    kwargs['short'] = short
    burn_times = (300, 600, 900, 1200)
    thrusts = [100] if short else (25, 50, 75, 100)
    p_cc_range = range(3, 11)
    labels = ('EP', 'GG')
    classes = (KwakFixElectricPumpCycle, KwakFixGasGeneratorCycle) if kwargs['kwak'] else (ElectricPumpCycle, GasGeneratorCycle)
    extra_kwargses = (args.ep_arguments, args.gg_arguments)
    enginedict = {}
    for label, engine_class, extra_kwargs in zip(labels, classes, extra_kwargses):
        for burn_time in burn_times:
            for thrust in thrusts:
                arguments['burn_time'] = burn_time
                arguments['thrust'] = thrust * 1e3
                arguments['verbose'] = kwargs['verbose']
                enginedict[f'{label} tb:{burn_time}, ft:{thrust}'] = [
                    engine_class(combustion_chamber_pressure=p_cc * 1E6, **arguments, **extra_kwargs)
                    for p_cc in p_cc_range
                ]
    plot_isp_ratio(dir_name=kwargs['dir_name'], savefig=kwargs['savefig'], kwak=kwargs['kwak'])
